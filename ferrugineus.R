...
str(bugs.data <- list (y=y, B=B, nind=nind, nsites=nsites, midpt=midpt, delta=delta, dclass=dclass, site=site, nz=nz, nD=nD, groupsize=groupsize,elevation=elevation,agroflorestal=agroflorestal,agroganadero=agroganadero,noto=noto,otros=otros,urbano=urbano ))
...
cat("
    ...
    beta4 ~ dunif(-10,10)             # noto slope
    beta5 ~ dunif(-10,10)             # otros slope
    beta6 ~ dunif(-10,10)             # urbano slope
    ...
    for(s in 1:nsites){                  # Loop over sites
     ...
     N[s] ~ dpois(lambda[s])
     log(lambda[s])<- beta0 + beta1*elevation[s] + beta2*agroflorestal[s] + beta3*agroganadero[s] + beta4*noto[s] + beta5*otros[s] + beta6*urbano[s]
