...
We then model the number of groups as a function of covariates. The model for number of groups is G~i~ ~ Poisson(D~i~A~i~), where D~i~ = covariates and A~i~ = area sampled in site _i_. A~i~ is calculated using the habitat-specific estimated EDR as $A_i = L_i * (2EDR_i)$, where $L$ is the length of each site (i.e. the transect). A~i~ is added to the model as an offset.
...
set.seed(333)
VIP.habitat.season <- vip(Y=x$count, X=X,  V=2, Z=Z, truncate=TRUE, hessian=TRUE, method="DE")
```
...
plot(gof0, type="b", col=2,xlim=c(0,max(c(gof,gof0))),ylim=c(0,max(c(gof,gof0))))
points(jitter(gof,0.1,0), type="b", col=4)
abline(0, 1, lty=2)
...
Estimated model coefficients (Table 13) indicate that group sizes are larger in urban ($\beta$ = `r round(coef(VIP.habitat.season)[2],3)`) and agropastoral ($\beta$ = `r round(coef(VIP.habitat.season)[3],3)`) habitats when compared to other natural habitats. Moreover, the probability of there being extra pairs (i.e. the V-inflation probability, $\phi$) is smaller in the non-breeding season ($\beta$ = `r round(coef(VIP.habitat.season)[5],3)`).   
...
VIP.model.fun<-function(df){
  ...
  VIP.habitat.season <- vip(Y=df$count, X=X,  V=2, Z=Z, truncate=TRUE, hessian=T, method="DE", init=inits)
}
...
The mean expected abundance ($E$) can be predicted for each surveyed site $i$ as $E_{i} = (\phi_{i} 2 + (1 - \phi_{i})\mu_{i})N_{i}$, where $N$ is the mean number of groups, $\mu$ is the mean group size and $\phi$ is the V(=2)-inflation probability (i.e. the probability that a given group will be a pair).
...
```{r, echo=FALSE, warning=F,message=F}
library(MASS)
# predict number of groups  for each site surveyed

modmat<-model.matrix(~habitat+elevation+I(elevation^2)+season+as.factor(year),sites)

lambda_mvsamps<-exp(modmat %*% cbind(coef(ngroup.hab.season.ele2.year),t(mvrnorm(n=1000,mu=coef(ngroup.hab.season.ele2.year),Sigma=vcov(ngroup.hab.season.ele2.year))))+log(sites$A))


# predict mu and phi from Gsize model for all sites surveyed (including those with 0 groups counted)
mu_phi_fun<-function(object){
  
  newX <- model.matrix(~Urban+Agropastoral,sites) # get predictor values from the 'sites' dataframe, that includes all sites surveyed (object sites)
  newZ <- model.matrix(~season, sites) # same as above for season
  ...
  mu <- as.vector(linkinvx(newX %*% parms[1:kx] ))  
  mu_mvsamps<- linkinvx(newX %*% t(mvrnorm(n=1000,mu=parms[1:kx],Sigma=object$vcov[1:kx,1:kx])))

  
  phi <- as.vector(linkinvz(newZ %*% parms[(kx + 1):(kx + kz)] )) 
  phi_mvsamps<- linkinvz(newZ %*% t(mvrnorm(n=1000,mu=parms[(kx + 1):(kx + kz)],Sigma=object$vcov[(kx + 1):(kx + kz),(kx + 1):(kx + kz)])))

  
  out<-list(mu=mu,mu_mvsamps=mu_mvsamps,phi=phi,phi_mvsamps= phi_mvsamps)
  return(out)
}

mu_phi<-mu_phi_fun(VIP.habitat.season)



E_mvsamps<-matrix(NA,2901,1000)
for(i in 1:1000){
  E_mvsamps[,i]<- (mu_phi$phi_mvsamps[,i]*2 + (1-mu_phi$phi_mvsamps[,i])*mu_phi$mu_mvsamps[,i])*lambda_mvsamps[,i+1]
}
E_CIs <- t( apply(E_mvsamps, 1, quantile, c(0.025,0.975)))
colnames(E_CIs)<-c("E_2.5","E_97.5")


pred_df<-cbind(ngroup.hab.season.ele2.year$data,lambda=lambda_mvsamps[,1], mu=mu_phi$mu,phi=mu_phi$phi) 
pred_df$E<-(pred_df$phi*2 + (1-pred_df$phi)*pred_df$mu)*pred_df$lambda
pred_df<-cbind(pred_df,E_CIs)


sumE<-aggregate(pred_df$E,list(pred_df$habitat,pred_df$year,pred_df$season),FUN=sum)
sumE_2.5<-aggregate(pred_df$E_2.5,list(pred_df$habitat,pred_df$year,pred_df$season),FUN=sum)
sumE_97.5<-aggregate(pred_df$E_97.5,list(pred_df$habitat,pred_df$year,pred_df$season),FUN=sum)
sumA<-aggregate(pred_df$A,list(pred_df$habitat,pred_df$year,pred_df$season),FUN=sum)

result<-cbind(sumE[,1:3],"Density (individuals/ha)" =round(sumE$x/(sumA$x*100),3),"Density (2.5% CI)" =round(sumE_2.5$x/(sumA$x*100),3),"Density (97.5% CI)" =round(sumE_97.5$x/(sumA$x*100),3))
colnames(result)[1:3]<-c("Habitat","Year","Season")
kable(result,caption ="\\textit{E. ferrugineous} mean density predictions for each habitat type, year and season")
...
The model for number of groups is G~i~ ~ Poisson(D~i~A~i~), where D~i~ = covariates and A~i~ = area sampled in site _i_. A~i~ is calculated using the habitat-specific estimated EDR, and is added to the model as an offset.
...
Finally, we assess year effects by adding a year covariate (2013-2016).
```{r, echo=FALSE, warning=F,message=F}
ngroup.habXjdate.ele.year <- glm(ngroups~elevation+habitat*jdate+as.factor(year), family=poisson, data=sites, offset=log(sites$A))
...
set.seed(321)
VIP.habitat.season <- vip(Y=x$count, X=X, V=2, Z=Z, truncate=TRUE, hessian=T, method="DE", init=c(coef(glm(x$count~x$Urban+x$Agropastoral,family=poisson)),0,0))
...
Confidence intervals (CI) based on estimated standard errors can be obtained (Table 26):  

```{r, echo=FALSE, warning=F,message=F}
#kable(confint(VIP.habitat.season),digits = 3,caption="\\textit{E. leptorhynchus} group size model CI")
```


Estimated model coefficients (Table 25) indicate that group sizes are slightly smaller in urban ($\beta$ = `r round(coef(VIP.habitat.season)[2],3)`) and larger in agropastoral ($\beta$ = `r round(coef(VIP.habitat.season)[3],3)`) habitats when compared to other natural habitats. Moreover, the probability of there being extra pairs (i.e. the V-inflation probability, $\phi$) is smaller in the non-breeding season ($\beta$ = `r round(coef(VIP.habitat.season)[5],3)`). 

The very low estimated standard errors indicate that there may be some issues with this model. This is further corroborated by the different coefficient and CI values obtained using the bootstrap mean and quantile approach.
...
VIP.boot.coefs<-rbind(coef(VIP.habitat.season), t(sapply(VIPs, coef)))
coefCIs<-t(rbind(apply(VIP.boot.coefs[-1,], 2, mean), apply(VIP.boot.coefs[-1,], 2, quantile, c(0.025,0.975))))
colnames(coefCIs)<-c("Estimate","2.5%","97.5%")


kable(round(coefCIs,3),caption ="Coefficients and 95% CI based on bootstrap mean and quantiles for \\textit{E. leptorhynchus} group size models")

```

Because of the lack of concordance between direct estimates and the bootstrap approach, indicative of issues with the model, and since there is only a very small difference in goodness-of-fit compared to the null model (Figure 6), we report density prediction using the null model.
...
modmat<-cbind(model.matrix(~elevation+habitat*jdate+as.factor(year),sites))

set.seed(123)
lambda_mvsamps<-exp(modmat %*% cbind(coef(ngroup.habXjdate.ele.year),t(mvrnorm(n=1000,mu=coef(ngroup.habXjdate.ele.year),Sigma=vcov(ngroup.habXjdate.ele.year))))+log(sites$A))
...
mu_phi_fun<-function(object){
  ...
  newX <- model.matrix(~1,sites) # get predictor values from the 'sites' dataframe, that includes all sites surveyed (object sites)
  newZ <- model.matrix(~1, sites) # same as above for season
...
result<-cbind(sumE[,1:3],"Density (individuals/ha)" =round(sumE$x/(sumA$x*100),3),"Density (2.5% CI)" =round(sumE_2.5$x/(sumA$x*100),3),"Density (97.5% CI)" =round(sumE_97.5$x/(sumA$x*100),3))
colnames(result)[1:3]<-c("Habitat","Year","Season")
kable(result,caption ="\\textit{E. leptorhynchus} mean density predictions for each habitat type, year and season")
