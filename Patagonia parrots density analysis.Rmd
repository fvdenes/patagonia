...
newdata<-data.frame(habitat=sites$habitat,elevation=sites$elevation,season=sites$season,year=as.factor(sites$year),offset=0)
lambda<- predict(ngroup.hab.season.ele2.year, type="response",newdata=newdata)

modmat<-model.matrix(~habitat+elevation+I(elevation^2)+season+as.factor(year),sites)

lambda_mvsamps<-exp(modmat %*% t(mvrnorm(n=1000,mu=coef(ngroup.hab.season.ele2.year),Sigma=vcov(ngroup.hab.season.ele2.year)))+log(sites$A))
...
mu_phi_fun<-function(object){
  ...
  newoffsetx <- log(sites$A) # sames as above for the surveyed area (A) - NOT SURE IF THIS SHOULD BE 0 INSTEAD
  ...
  mu <- as.vector(linkinvx(newX %*% parms[1:kx] + newoffsetx))  
  mu_mvsamps<- linkinvx(newX %*% t(mvrnorm(n=1000,mu=parms[1:kx],Sigma=object$vcov[1:kx,1:kx]))+ newoffsetx)

  
  phi <- as.vector(linkinvz(newZ %*% parms[(kx + 1):(kx + kz)] + object$offsetz)) # this offset was 0 in the model
  phi_mvsamps<- linkinvz(newZ %*% t(mvrnorm(n=1000,mu=parms[(kx + 1):(kx + kz)],Sigma=object$vcov[(kx + 1):(kx + kz),(kx + 1):(kx + kz)]))+ object$offsetz)
...
for(i in 1:1000){
  E_mvsamps[,i]<- (mu_phi$phi_mvsamps[,i]*2 + (1-mu_phi$phi_mvsamps[,i])*mu_phi$mu_mvsamps[,i])*lambda_mvsamps[,i]
}
E_CIs <- t( apply(E_mvsamps, 1, quantile, c(0.025,0.975)))
colnames(E_CIs)<-c("E_2.5","E_97.5")


pred_df<-cbind(ngroup.hab.season.ele2.year$data,lambda=lambda, mu=mu_phi$mu,phi=mu_phi$phi) 
...
library(MASS)
# predict number of groups  for each site surveyed
newdata<-data.frame(habitat=sites$habitat,elevation=sites$elevation,jdate=sites$jdate,year=as.factor(sites$year),offset=0)
lambda<- predict(ngroup.habXjdate.ele.year, type="response",newdata=newdata)

modmat<-cbind(model.matrix(~elevation+habitat*jdate+as.factor(year),sites))

set.seed(123)
lambda_mvsamps<-exp(modmat %*% t(mvrnorm(n=1000,mu=coef(ngroup.habXjdate.ele.year),Sigma=vcov(ngroup.habXjdate.ele.year)))+log(sites$A))
...
mu_phi_fun<-function(object){
  ...
  newoffsetx <- 0 # The null model (mod0) does not have an offset. Not sure if this is correct though.
