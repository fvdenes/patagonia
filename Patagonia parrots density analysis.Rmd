...
Estimated model coefficients indicate that group sizes are larger in urban ($\beta$ = 1.75) and agropastoral ($\beta$ = 0.61) habitats when compared to other natural habitats. Moreover, the probability of there being extra pairs (i.e. the V-inflation probability, $\phi$) is smaller in the non-breeding season ($\beta$ = -3.47). 

Alternatively, we can estimate coefficient  confidence intervals based on quantiles using bootstrap (with n=250):
...
VIP.boot.coefs[,1]-coef(VIP.habitat.season)
...
kable(round(coefCIs,3),caption ="Estimated coefficients and 90% CI based on bootstrap quantiles for \\textit{E. ferrugineous} group size models")
