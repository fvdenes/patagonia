...
Considence intervals (CI) based on estimated standard errors can be obtained (Table 14):
...
quant<-apply(phi,1,quantile,probs=c(0.025,0.975))
...
The models 'ngroup.hab.ele' (with linear elevation effect) and 'ngroup.hab.ele2' (with quadratic effect) are equally parsimonious (Table 18), but the quadratic term in the latter is mostly uniformative (Tables 19-20). Given these are nested models, we drop the quadratic term and continue with the model with habitat and linear elevation effects.
